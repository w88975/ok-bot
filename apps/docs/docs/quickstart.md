...
在仓库根目录运行以下命令，可在终端与 agent 对话（流式输出 + 工具调用进度）：

```bash
pnpm --filter @ok-bot/core build
node start.mjs
```

`start.mjs` 使用 `OnEvent` 回调，按事件类型在终端展示思考、文本、工具调用与实时 stdout。修改脚本顶部的 `WORKSPACE`、`provider` 等可切换工作目录和模型。
